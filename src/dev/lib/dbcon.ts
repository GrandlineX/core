...
export default function jestDb() {
  describe('dbcon', () => {
    ...
    describe('CoreDBPrefab delegating methods', () => {
      ...
      test('prepare', () => {
        prefab = kernel.getChildModule('testModule').getDb() as TestPrefab;
        wrapper = prefab.getEntityWrapper('TestEntity');
      });
      ...
      test('createEntity and getEntityById', async () => {
        ...
        ent1 = await wrapper.createObject(ent1);
        expect(ent1.e_id).not.toBeNull();
        const found = await wrapper.getObjById(ent1.e_id);
      ...
      test('getEntityBulkById', async () => {
        ...
        ent2 = await wrapper.createObject(ent2);
        const bulk = await wrapper.getObjByIdBulk([ent1.e_id, ent2.e_id]);
        expect(bulk.length).toBe(2);
      });
      test('updateEntity', async () => {
        await wrapper.updateObject(ent1.e_id, {
          name: 'PrefabUpdated',
        });
        expect((await wrapper.getObjById(ent1.e_id))?.name).toBe(
          'PrefabUpdated',
        );
      });
      test('updateBulkEntity', async () => {
        expect(
          await wrapper.updateObjectBulk([ent1.e_id, ent2.e_id], {
            name: 'BulkPrefab',
          }),
        ).toBe(true);
      });
      test('getEntityList', async () => {
        expect(Array.isArray(await wrapper.getObjList())).toBe(true);
      });
      test('findEntity', async () => {
        const found = await wrapper.findObj({
          name: 'BulkPrefab',
        });
        expect(found).not.toBeNull();
      });
      test('countEntity', async () => {
        expect(await wrapper.count()).toBeGreaterThanOrEqual(2);
      });
      test('initEntity', async () => {
        expect(await prefab.initEntity('_CovTmp', new TestEntity())).toBe(true);
      });
      test('deleteEntityById', async () => {
        expect(await wrapper.delete(ent1.e_id)).toBe(true);
      });
      test('deleteEntityBulkById', async () => {
        expect(await wrapper.deleteBulk([ent2.e_id])).toBe(true);
      });
      test('countEntity - after', async () => {
    ...
    describe('DBEdgeCase edge cases', () => {
      ...
      describe('aFilter modes via getEntityList', () => {
        ...
        test('updateEntity - entity not found throws', async () => {
          expect(
            await wrapper.updateObject('nonexistent-id', { name: 'X' }),
          ).toBe(false);
        });
